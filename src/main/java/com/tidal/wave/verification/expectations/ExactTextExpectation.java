...
public class ExactTextExpectation extends Expectation {
    ...
    public void assertion(boolean isVisible, boolean isMultiple, List<By> locatorSet) {
        ...
        result = new FluentWait<>(executor)
                ...
                .withMessage(String.format("Expected value '%s' is not equal to actual value %s", value, executor.isVisible(isVisible).withMultipleElements(isMultiple).usingLocator(locatorSet).invokeCommand(FindTextData.class, FIND_TEXT_DATA)))
                .until(e -> e
    ...
    public void orElseFail() {
        if (!result) {
            throw new ExpectationFailure(String.format("Expected value '%s' is not equal to actual value %s", value, executor.isVisible(isVisible).withMultipleElements(isMultiple).usingLocator(locatorSet).invokeCommand(FindTextData.class, FIND_TEXT_DATA)));
        }
    }
}
