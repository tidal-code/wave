...
public class ElementNotPresentExpectation extends Expectation {
    ...
    public void assertion(boolean isVisible, boolean isMultiple, List<By> locatorSet) {
        ...
        result = new FluentWait<>(executor)
                ...
                .withMessage(String.format("Expected condition failed : Element %s expected to be not present but one or more was present in the DOM", locatorSet.get(0)))
                .until(e -> (int) e
    ...
    public void orElseFail() {
        if (!result) {
            throw new ExpectationFailure(String.format("Expected condition failed : Element %s expected to be not present but was present", byLocator));
        }
    }
}
