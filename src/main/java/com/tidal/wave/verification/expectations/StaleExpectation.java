...
public class StaleExpectation extends Expectation {
    ...
    public void assertion(boolean isVisible, boolean isMultiple, List<By> locatorSet) {
        ...
        result = newFluentWait
                ...
                .withMessage(String.format("Expected condition failed : Element %s expected to stale but was not", locatorSet.get(0)))
                .until(e -> !(boolean) (e
    ...
    public void orElseFail() {
        if (!result) {
            throw new ExpectationFailure(String.format("Expected condition failed : Element %s expected to be stale but was not", byLocator));
        }
    }
}
